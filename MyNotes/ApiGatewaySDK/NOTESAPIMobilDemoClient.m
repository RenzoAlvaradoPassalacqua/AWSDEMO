...
#import "NOTESAPIMobilDemoClient.h"
...
@interface NOTESAPIMobilDemoClient()
...
@implementation NOTESAPIMobilDemoClient

static NSString *const AWSInfoClientKey = @"NOTESAPIMobilDemoClient";
...
+ (instancetype)defaultClient {
    ...
    static NOTESAPIMobilDemoClient *_defaultClient = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _defaultClient = [[NOTESAPIMobilDemoClient alloc] initWithConfiguration:serviceConfiguration];
    });
...
+ (void)registerClientWithConfiguration:(AWSServiceConfiguration *)configuration forKey:(NSString *)key {
    ...
    [_serviceClients setObject:[[NOTESAPIMobilDemoClient alloc] initWithConfiguration:configuration]
                        forKey:key];
}

+ (instancetype)clientForKey:(NSString *)key {
    @synchronized(self) {
        NOTESAPIMobilDemoClient *serviceClient = [_serviceClients objectForKey:key];
        if (serviceClient) {
        ...
        if (serviceInfo) {
            AWSServiceConfiguration *serviceConfiguration = [[AWSServiceConfiguration alloc] initWithRegion:serviceInfo.region
                                                                                        credentialsProvider:serviceInfo.cognitoCredentialsProvider];
            [NOTESAPIMobilDemoClient registerClientWithConfiguration:serviceConfiguration
                                                    forKey:key];
        }
...
- (instancetype)initWithConfiguration:(AWSServiceConfiguration *)configuration {
    if (self = [super init]) {
        ...
        NSString *URLString = @"https://c550icozi9.execute-api.us-east-2.amazonaws.com/Develop";
